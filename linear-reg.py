...
>>>>>>> 6c0f2c2d67e470ce9752168ba3d4e6957f8a0b8b
...
What is the nean distance across all trips?    Answer: 8.2895 miles
How many cab companies are in the dataset?     Answer: 31
What is the most frequent payment type?        Answer: Credit Card
Are any features missing data?                 Answer: No
'''
# You should be able to find the answers to the questions about the dataset 
# by inspecting the table output after running the DataFrane describe method.

# Run this code cell to verify your answers.
# What is the maximum fare?
print(training_df.columns)
max_fare = training_df['FARE'].max()
if 'FARE' in training_df.columns:
    max_fare = training_df['FARE'].max()
else:
    print("Column 'FARE' not found in DataFrame.")

print("What is the maxinun frare? \t\t\t\tAnswer: ${:.2f}".format(max_fare))

# What is the mean distance across all trips?
mean_distance = training_df['TRIP_MILES'].mean()
print("What is the mean distance across all trips? \t\tAnswer : {mean:.4f}miles".format(mean = mean_distance))

# How many cab compnies are in the dataset?
num_unique_companies = training_df['COMPANY'].nunique()
print("How many cab companies are in the dataset? \t\tAnswer : {number}".format(number = num_unique_companies))

# What is the most frequent payment type?
most_freq_payment_type = training_df['PAYMENT_TYPE'].value_counts().index[0]
...
def make_plots(df, feature_names, label_name, model_output):
    ...
    # Check if feature_names has at least 2 elements
    if len(feature_names) < 2:
        raise ValueError("feature_names must contain at least two elements")

    fig = make_subplots(rows=1, cols=2, subplot_titles=("Loss Curve", "Model Plot"), specs=[[{"type": "scatter"}, {"type": "scatter"}]])

    # Add traces to the figure
    fig.add_trace({
        'type': 'scatter',
        'x': random_sample[feature_names[0]],
        'y': random_sample[label_name],
        'mode': 'markers'
    }, row=1, col=1)

    fig.add_trace({
        'type': 'scatter',
        'x': random_sample[feature_names[1]],
        'y': model_output[:sample_size],
        'mode': 'markers'
    }, row=1, col=2)
...
def make_plots(df, feature_names, label_name, model_output, sample_size = 200):
    ...
    fig = make_subplots(rows=1, cols=2,subplot_titles=("Loss Curve", "Model Plot"), specs=[({"type": model_plot_type})])
...
def plot_loss_curve(epochs, rmse, fig):
    ...
    fig.append_trace(curve.date[0], row=1, col=1)
    fig.updat_xaxes(title_text = "Epoch", row=1, col=1)
    fig.update_yaxes(title_text = "Root Mean Error", row=1, col=1, range=[rmse.min()*0.8, rmse.max()])
    return
...
# Define feature names with at least two features
feature_names = ['TRIP_MILES', 'TRIP_SECONDS']
